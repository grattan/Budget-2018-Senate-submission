...
But while the components of the plan will be \emph{introduced} in a staggered way, their \emph{impact} will be permanent and ongoing. So although the last tranche only comprises 29 per cent of the cost of the package over eleven years,\footcite[][2]{Treasury2018c}
...
At present, the top 20 per cent of income earners -- who earn \Sexpr{grattan_percent(p1718[.(5)][["inc"]])} of taxable income -- pay \Sexpr{grattan_percent(p1718[.(5)][["tax"]])} per cent of income tax. As shown in \Vref{tbl:1}, without changes to income tax rates, bracket creep will make the system \emph{less} progressive, and they will only pay \Sexpr{grattan_percent(sum(tail(share_tot_inc_tax[["2027-28 baseline"]], 3)))}.



Although many think of ``bracket creep'' as a problem for high income earners, it most affects those in the middle of the income distribution (around \$\Sexpr{comma(s1718[, median(Taxable_Income)])} taxable income per year). They are affected not because they move into a higher tax bracket, but because increases in income as a result of inflation are taxed at their \emph{marginal} tax rate, which is typically much higher than their \emph{average} tax rate. Overall, bracket creep affects people more if the percentage point difference between their average and marginal tax rate is large, and if their total income is relatively small.
  ...
  fwrite("atlas/avg-tax-rates-v-percentile.csv") %>%
  {
    grplot(data = ., aes(x = Percentile, y = value, color = variable, group = variable)) + 
      ...
      geom_label_repel(data = subset(., grepl("^Net", variable)),
                       # between the lines
                       xlim = c(25, NA),
                       ylim = c(0, max(.[grepl("^Net", variable)][["value"]])),
                       aes(label = label),
                       label.size = NA,
                       family = "helvet",
                       size = 6.14,
